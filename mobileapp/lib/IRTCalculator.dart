class IRTCalculator{
  ...
  void calculate(double salarioBase, [double totalsubsidioTributavel = 0]){
    ...
    final double grossIncome = salarioBase + totalsubsidioTributavel;
    Division division = this.getDivision(grossIncome);
    
    //INSS is always 3% of gross income
    this.inss = grossIncome * 0.03;
    this.taxableAmount = grossIncome - this.inss;
    this.irt = division.fixedParcel + ((taxableAmount - division.excess) * (division.tax/100));
    this.netSalary = taxableAmount - irt;
  }
  
  //return the division to operate
  Division getDivision(double grossIncome){
